(function() {
  ...
  Tuner = function(audioContext, buffer, bufferFiller) {
    var error, gauss, hp, i, lp, success;
    ...
    if (audioContext.createBiquadFilter) {
		...
		var biquadLow = new Biquad(DSP.LPF, sampleRate);
		}
    if (audioContext.createBiquadFilter) {
    	...
    	var biquadHigh = new Biquad(DSP.HPF, sampleRate);
		}    
    success = function(stream) {
      ...
      try {
        ...
        process = function() {
          ...
          var b, bufferCopy, diff, downsampled, firstFreq, freq, interp, left, noiseThrehold, note, volume, p, peak, peaks, q, right, s, secondFreq, spectrumPoints, thirdFreq, upsampled, x, _i, _j, _k, _l, _len, _m, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
          bufferCopy = (function() {
      ...
      display = {
        draw: function(note, diff,freq) {
		  ...
		  globals.callPaperTuner(note,freq,diff); // calls function in paperTuner
        }
      };
    ...
    error = function(e) {
      console.log(e);
      alert('To use this tuner with a real guitar, use the latest version of Google Chrome with "Web Audio API" enabled in chrome://flags');
      return alert('We need microphone access to get started!');
    };
